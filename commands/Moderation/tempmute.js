...
module.exports.run = async (client, message, args) => {
  ...
  if(!message.member.hasPermission('MANAGE_GUILD')) return message.channel.send(`${client.language["mute"].noPermission}`);
    if(message.content.split(" ").slice(2) < 1) return;
    const time = message.content.split(" ").slice(2);
    if(isNaN(time)) {
    	message.channel.send(`${client.language["mute"].invalidDuration}`);
    	return;
    }

    if (message.mentions.users.size < 1) {
      message.channel.send(`${client.language["mute"].noMention}`);
      return;
    }
    ...
    if (!message.guild.member(client.user).hasPermission('MANAGE_ROLES_OR_PERMISSIONS')) return message.channel.send(`${client.language["mute"].noRolesPermission}`);

    	// Mute command

      let mutedMessage = client.language["tempmute"].muted;
    	...
    	if(!mutedRole) {
          ...
          setTimeout(() => {
            ...
            if(!member.roles.has(mutedRoleCreated.id)) {
              message.channel.send(`${client.language["mute"].noRoleAsigned}`);
              return;
            }
     				member.removeRole(mutedRoleCreated.id);

            let unmutedMessage = client.language["mute"].unmuted;
